...
import { getRestFeedbackSettings, triggerRestFinishedFeedback } from "@/services/rest-feedback.service";
import { endRestTimerLiveActivity, startRestTimerLiveActivity } from "@/services/rest-timer-live-activity.service";
...
export const useRestTimerStore = create<RestTimerState>((set, get) => ({
  ...
  hydrate: async () => {
    try {
      ...
      startRestTimerLiveActivity({ endAt: parsed.endAt, duration: parsed.duration });
    } catch (error) { console.log("REST TIMER HYDRATE ERROR =", error); set({ hydrated: true }); }
  },
  ...
  start: async (seconds) => {
    ...
    startRestTimerLiveActivity({ endAt, duration: safeSeconds });
  },
  ...
  sync: () => {
    ...
    if (remaining <= 0) {
      ...
      void clearStorage();
      void endRestTimerLiveActivity();
      void getRestFeedbackSettings().then((settings) => triggerRestFinishedFeedback(settings)).catch(() => undefined);
      console.log("REST TIMER FINISHED =", previousEndAt);
      return;
    }
    set({ remaining });
  },
