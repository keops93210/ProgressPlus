...
export default function CoachNextSetCard({
  ...
  const nextReps = shouldConsolidate
    ...
    : shouldIncreaseWeight ? minReps : Math.min(maxReps, reps + 1);
  ...
  const confidenceMessage = hardEffort
    ? "RIR très bas : on protège la prochaine série plutôt que de forcer."
    : recoveryLow
      ? "La récupération limite volontairement la progression."
      : shouldIncreaseWeight
        ? "Performance et récupération cohérentes."
        : "Progression possible sans surcharge automatique.";
  ...
  const message = hardEffort
    ? `RIR ${rir}. Ne cherche pas à augmenter la charge sur la prochaine série.`
    : recoveryLow
      ? `Ta récupération est basse. Garde ${weight} kg et reste dans une zone maîtrisée.`
      : shouldIncreaseWeight
        ? `${maxReps} reps atteintes avec suffisamment de marge. Progress+ prépare ${nextWeight} kg.`
        : isBelowTarget
    ...
    <View style={styles.card}>
      <View style={styles.header}>
        <View style={styles.titleRow}>
          {hardEffort || recoveryLow ? <TrendingDown size={17} color={Colors.textSecondary} /> : shouldIncreaseWeight ? <TrendingUp size={17} color={Colors.primary} /> : <Check size={17} color={Colors.primary} />}
          <Text style={styles.eyebrow}>COACH PROGRESS+</Text>
        </View>
        <View style={styles.headerRight}>
          <View style={[styles.decisionBadge, shouldIncreaseWeight && styles.decisionBadgePrimary]}><Text style={[styles.decisionText, shouldIncreaseWeight && styles.decisionTextPrimary]}>{decision}</Text></View>
          <Text style={styles.setLabel}>{nextSet <= totalSets ? `SÉRIE ${nextSet}/${totalSets}` : "DERNIÈRE SÉRIE"}</Text>
        </View>
      </View>
      <View style={styles.setProgress}>{Array.from({ length: Math.max(1, totalSets) }).map((_, index) => { const setNumber = index + 1; return <View key={setNumber} style={[styles.setDot, setNumber < nextSet && styles.setDotCompleted, setNumber === nextSet && styles.setDotCurrent]} />; })}</View>
      <View style={styles.qualityRow}>
        ...
        <View style={styles.qualityScore}><Text style={styles.qualityScoreText}>{hardEffort ? "!" : reps >= minReps ? "✓" : "!"}</Text></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  titleRow: { flexDirection: "row", alignItems: "center", gap: 7 }, eyebrow: { color: Colors.primary, fontSize: 10, fontWeight: "900", letterSpacing: 1.2 },
  headerRight: { flexDirection: "row", alignItems: "center", gap: 7 }, decisionBadge: { paddingHorizontal: 7, paddingVertical: 4, borderRadius: 999, backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border }, decisionBadgePrimary: { backgroundColor: Colors.primary + "18", borderColor: Colors.primary + "45" }, decisionText: { color: Colors.textSecondary, fontSize: 8, fontWeight: "900", letterSpacing: 0.6 }, decisionTextPrimary: { color: Colors.primary }, setLabel: { color: Colors.textSecondary, fontSize: 10, fontWeight: "800", letterSpacing: 0.8 },
  setProgress: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: 13 }, setDot: { flex: 1, height: 4, borderRadius: 999, backgroundColor: Colors.border }, setDotCompleted: { backgroundColor: Colors.primary + "65" }, setDotCurrent: { backgroundColor: Colors.primary, height: 6 },
  qualityRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 15 }, qualityCopy: { flex: 1 }, qualityEyebrow: { color: Colors.textSecondary, fontSize: 9, fontWeight: "900", letterSpacing: 1 }, qualityValue: { color: Colors.text, fontSize: 16, fontWeight: "900", marginTop: 3 }, qualityScore: { width: 30, height: 30, borderRadius: 15, alignItems: "center", justifyContent: "center", backgroundColor: Colors.primary + "18", borderWidth: 1, borderColor: Colors.primary + "35" }, qualityScoreText: { color: Colors.primary, fontSize: 15, fontWeight: "900" }, qualityMessage: { color: Colors.textSecondary, fontSize: 11, lineHeight: 16, marginTop: 4 },
