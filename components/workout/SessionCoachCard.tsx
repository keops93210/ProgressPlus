    ...
    <View style={styles.card}>
      ...
      <View style={styles.footer}>
        <View>
          ...
          <Text style={[styles.score, { color: tone.color }]}>{decision.qualityScore}/100</Text>
        </View>
        <View style={[styles.action, { borderColor: tone.color }]}>
          <Text style={[styles.actionText, { color: tone.color }]}>
            {decision.nextSessionAction === "progress" ? "PROGRESSER" : decision.nextSessionAction === "consolidate" ? "CONSOLIDER" : decision.nextSessionAction === "reduce" ? "RÉDUIRE" : "RÉCUPÉRER"}
          </Text>
        </View>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  headerText: { flex: 1 },
  eyebrow: { color: "#A78BFA", fontSize: 10, fontWeight: "800", letterSpacing: 1.2 },
  title: { color: "#F5F5F7", fontSize: 19, fontWeight: "800", marginTop: 3 },
  confidence: { alignItems: "flex-end" },
  confidenceValue: { color: "#F5F5F7", fontSize: 15, fontWeight: "800" },
  confidenceLabel: { color: "#777985", fontSize: 10, marginTop: 2 },
  message: { color: "#B9BAC3", fontSize: 14, lineHeight: 21 },
  footer: { flexDirection: "row", alignItems: "flex-end", justifyContent: "space-between", paddingTop: 4 },
  footerLabel: { color: "#777985", fontSize: 9, fontWeight: "800", letterSpacing: 1 },
  score: { fontSize: 24, fontWeight: "900", marginTop: 2 },
  action: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 11, paddingVertical: 7 },
  actionText: { fontSize: 10, fontWeight: "900", letterSpacing: .6 },
});
