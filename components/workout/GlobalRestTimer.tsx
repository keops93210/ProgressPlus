...
export default function GlobalRestTimer() {
  ...
  const restFinished = remaining === 0;
  const restEnding = remaining > 0 && remaining <= Math.min(15, Math.max(5, Math.round(duration * 0.1)));
  const stateColor = restFinished || restEnding ? Colors.success : Colors.primary;
  const stateLabel = restFinished ? "PRÊT POUR LA SUITE" : restEnding ? "BIENTÔT PRÊT" : "TEMPS DE REPOS";
      ...
      <View style={styles.shadow}>
        <View style={styles.pill}>
          <Pressable style={styles.main} onPress={() => setExpanded((value) => !value)}>
            <View style={[styles.iconWrap, { backgroundColor: stateColor }]}>
              {restFinished ? <Check size={20} color="#FFFFFF" strokeWidth={2.8} /> : <Timer size={19} color="#FFFFFF" strokeWidth={2.5} />}
            </View>
            <View style={styles.labels}>
              <Text style={[styles.eyebrow, { color: restFinished || restEnding ? Colors.success : Colors.textSecondary }]}>{stateLabel}</Text>
              <Text style={styles.time}>{formatTime(remaining)}</Text>
            </View>
            <View style={styles.progressTrack}>
              <View style={[styles.progressFill, { width: `${progress * 100}%`, backgroundColor: stateColor }]} />
            </View>
          </Pressable>
        ...
        {expanded && (
          <View style={styles.expanded}>
            <Text style={styles.expandedTitle}>{restFinished ? "Repos terminé. Prêt pour la prochaine série." : `Encore ${formatTime(remaining)} avant la prochaine série`}</Text>
...
const styles = StyleSheet.create({
  ...
  shadow: { shadowColor: "#000", shadowOpacity: 0.35, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 20 },
  pill: { minHeight: 64, borderRadius: 22, backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, flexDirection: "row", alignItems: "center", overflow: "hidden" },
  ...
  time: { color: Colors.text, fontSize: 24, fontWeight: "900", marginTop: 1, fontVariant: ["tabular-nums"] },
  progressTrack: { flex: 1, height: 5, borderRadius: 4, backgroundColor: Colors.border, overflow: "hidden", marginLeft: 10 },
