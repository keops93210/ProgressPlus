...
function ScorePicker({ title, hint, value, onChange }: {
  title: string;
  hint?: string;
  value: number;
  ...
  return (
    <View style={styles.question}>
      ...
      <View style={styles.options}>
        {options.map((option) => (
          <Pressable key={option.value} onPress={() => onChange(option.value)} style={[styles.option, value === option.value && styles.optionActive]}>
            <Text style={[styles.optionNumber, value === option.value && styles.optionNumberActive]}>{option.value}</Text>
          </Pressable>
        ))}
      </View>
...
export default function WorkoutCheckIn({ onComplete, onContinue }: Props) {
  ...
  const readyLabel = useMemo(() => {
    const score = Math.round((values.sleep + values.energy + values.mood + (6 - values.fatigue) + (6 - values.pain)) / 5);
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.button} onPress={handleSubmit}><Text style={styles.buttonText}>COMMENCER MA SÉANCE</Text></Pressable>
    </View>
  );
}
