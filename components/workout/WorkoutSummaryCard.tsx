...
export default function WorkoutSummaryCard({ summary }: { summary: WorkoutSummary }) {
  ...
  const qualityScore = Math.max(0, Math.min(100, Math.round(Number(summary.qualityScore) || 0)));
  const qualityLabel = qualityScore >= 85 ? "Excellente" : qualityScore >= 70 ? "Très bonne" : qualityScore >= 55 ? "Solide" : "À consolider";
  const qualityColor = qualityScore >= 85 ? Colors.success : qualityScore < 55 ? Colors.danger : Colors.primary;
  const coachDecision = getSummaryCoachDecision(summary);

  return (
    <View
      accessible
      accessibilityLabel={`Résumé de séance. Qualité ${qualityScore} sur 100, ${qualityLabel}. ${summary.totalSets} séries. ${Math.round(summary.volume).toLocaleString("fr-FR")} kilogrammes de volume. ${coachDecision.title}.`}
      style={styles.card}
    >
      ...
      <View style={styles.qualityHero}>
        ...
        <View style={[styles.scoreCircle, { borderColor: qualityColor }]}><Text style={[styles.score, { color: qualityColor }]}>{qualityScore}</Text><Text style={styles.scoreLabel}>/100</Text></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  qualityCopy: { flex: 1 }, qualityEyebrow: { color: Colors.textSecondary, fontSize: 9, fontWeight: "900", letterSpacing: 1 }, qualityLabel: { fontSize: 19, fontWeight: "900", marginTop: 3 }, qualityHint: { color: Colors.textSecondary, fontSize: 10, marginTop: 3 },
  scoreCircle: { width: 58, height: 58, borderRadius: 29, borderWidth: 2, alignItems: "center", justifyContent: "center" }, score: { fontSize: 20, fontWeight: "900" }, scoreLabel: { color: Colors.textSecondary, fontSize: 8, fontWeight: "800", marginTop: -2 }, message: { color: Colors.text, fontSize: 15, lineHeight: 21, fontWeight: "700", marginTop: 16 },
