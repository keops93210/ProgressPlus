...
export default function HomeScreen() {
  ...
  const score = globalScore.score == null ? "—" : Math.round(globalScore.score);
  ...
  return <SafeAreaView style={styles.safe}>
    <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={Colors.primary} />} contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.nextWorkout} activeOpacity={0.92} onPress={() => program && router.push({ pathname: "/(app)/program/[id]", params: { id: program.id } })}>
        <View style={styles.nextIcon}><Activity color={Colors.primaryLight} size={20} /></View>
        <View style={styles.nextCopy}><Text style={styles.nextEyebrow}>PROCHAINE SÉANCE</Text><Text style={styles.nextTitle}>{program?.name || "Commencer ton entraînement"}</Text><Text style={styles.nextMeta}>{program ? "Ton programme est prêt · aujourd'hui" : "Crée ton programme pour commencer"}</Text></View>
        <View style={styles.nextButton}><ChevronRight color="#fff" size={21} /></View>
      </TouchableOpacity>

      <View style={styles.levelCard}>
        <View style={styles.levelOrb}><Text style={styles.levelOrbText}>{ranking?.rank?.charAt(0)?.toUpperCase() ?? "P"}</Text></View>
        <View style={styles.levelCopy}><Text style={styles.levelLabel}>NIVEAU ACTUEL</Text><Text style={styles.levelTitle}>{ranking?.rank ?? "Bronze"}</Text><Text style={styles.levelSmall}>{ranking?.score ?? 0} XP · {rankPercent}% vers le prochain rang</Text></View>
        <Text style={styles.levelXp}>{ranking?.score ?? 0}</Text>
      ...
      <View style={styles.performanceCard}>
        ...
        <View style={styles.miniChart}><View style={[styles.bar, { height: 18 }]} /><View style={[styles.bar, { height: 28 }]} /><View style={[styles.bar, { height: 40 }]} /><View style={[styles.bar, { height: 32 }]} /><View style={[styles.bar, { height: 54 }]} /><View style={[styles.bar, { height: 46 }]} /><View style={[styles.bar, { height: 62 }]} /></View>
        <View style={styles.chartLabels}><Text>Lun</Text><Text>Mar</Text><Text>Mer</Text><Text>Jeu</Text><Text>Ven</Text><Text>Sam</Text><Text>Dim</Text></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  container: { paddingHorizontal: 18, paddingTop: 8, paddingBottom: 115, gap: 14 },
  loading: { flex: 1, alignItems: "center", justifyContent: "center", gap: 12, backgroundColor: "#07070A" },
  loadingText: { color: Colors.textSecondary },
  header: { paddingTop: 4, marginBottom: 2 },
  brandLine: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 },
  brand: { color: Colors.primaryLight, fontSize: 9, fontWeight: "900", letterSpacing: 2.1 },
  headerRow: { flexDirection: "row", alignItems: "center" },
  greeting: { color: "#FFFFFF", fontSize: 29, fontWeight: "900", letterSpacing: -0.7 },
  subtitle: { color: "#8F8F9D", fontSize: 12, marginTop: 3 },
  avatar: { width: 42, height: 42, borderRadius: 21, backgroundColor: "#111116", borderWidth: 1, borderColor: "#7650E8", alignItems: "center", justifyContent: "center" },
  avatarText: { color: Colors.primaryLight, fontSize: 17, fontWeight: "900" },
  nextWorkout: { minHeight: 86, borderRadius: 20, backgroundColor: "#111116", borderWidth: 1, borderColor: "#272631", padding: 13, flexDirection: "row", alignItems: "center" },
  nextIcon: { width: 42, height: 42, borderRadius: 14, backgroundColor: "#21143C", alignItems: "center", justifyContent: "center" },
  nextCopy: { flex: 1, marginLeft: 11 },
  nextEyebrow: { color: Colors.primaryLight, fontSize: 8, fontWeight: "900", letterSpacing: 1.2 },
  nextTitle: { color: "#FFFFFF", fontSize: 16, fontWeight: "900", marginTop: 3 },
  nextMeta: { color: "#858591", fontSize: 10, marginTop: 3 },
  nextButton: { width: 38, height: 38, borderRadius: 19, backgroundColor: Colors.primary, alignItems: "center", justifyContent: "center", marginLeft: 8 },
  levelCard: { minHeight: 96, backgroundColor: "#111116", borderRadius: 20, borderWidth: 1, borderColor: "#24232D", padding: 15, flexDirection: "row", alignItems: "center", position: "relative" },
  levelOrb: { width: 54, height: 54, borderRadius: 27, backgroundColor: "#24143F", borderWidth: 1, borderColor: "#774AE7", alignItems: "center", justifyContent: "center" },
  levelOrbText: { color: Colors.primaryLight, fontSize: 22, fontWeight: "900" },
  levelCopy: { flex: 1, marginLeft: 12 },
  levelLabel: { color: "#8E879D", fontSize: 8, fontWeight: "900", letterSpacing: 1.3 },
  levelTitle: { color: "#FFFFFF", fontSize: 20, fontWeight: "900", marginTop: 2 },
  levelSmall: { color: "#777782", fontSize: 9, marginTop: 3 },
  levelXp: { color: Colors.primaryLight, fontSize: 19, fontWeight: "900", alignSelf: "flex-start" },
  levelTrack: { position: "absolute", left: 15, right: 15, bottom: 10, height: 4, backgroundColor: "#292832", borderRadius: 3, overflow: "hidden" },
  levelFill: { height: "100%", backgroundColor: Colors.primary, borderRadius: 3 },
  scoreHeading: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end", marginTop: 5 },
  sectionKicker: { color: Colors.primaryLight, fontSize: 8, fontWeight: "900", letterSpacing: 1.3 },
  sectionTitle: { color: "#FFFFFF", fontSize: 17, fontWeight: "900", letterSpacing: -0.2 },
  seeMore: { color: Colors.primaryLight, fontSize: 10, fontWeight: "800" },
  weekHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 4 },
  sectionAction: { color: Colors.primaryLight, fontSize: 10, fontWeight: "800" },
  weekGrid: { flexDirection: "row", gap: 10 },
  rankMini: { width: 105, backgroundColor: "#111116", borderRadius: 18, borderWidth: 1, borderColor: "#24232D", padding: 13, justifyContent: "center" },
  rankMiniValue: { color: "#FFFFFF", fontSize: 25, fontWeight: "900", marginTop: 8 },
  rankMiniLabel: { color: "#777782", fontSize: 9, marginTop: 2 },
  coachCard: { backgroundColor: "#111116", borderRadius: 20, borderWidth: 1, borderColor: "#282632", padding: 15, flexDirection: "row" },
  coachIcon: { width: 43, height: 43, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  coachContent: { flex: 1, marginLeft: 11 },
  coachEyebrow: { fontSize: 8, fontWeight: "900", letterSpacing: 1.2 },
  coachTitle: { color: "#FFFFFF", fontSize: 16, fontWeight: "900", marginTop: 3 },
  coachMessage: { color: "#858591", fontSize: 11, lineHeight: 16, marginTop: 4 },
  coachAction: { fontSize: 10, fontWeight: "900", marginTop: 8 },
  performanceCard: { backgroundColor: "#111116", borderRadius: 20, borderWidth: 1, borderColor: "#24232D", padding: 15 },
  performanceHero: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  performanceKicker: { color: "#8E879D", fontSize: 8, fontWeight: "900", letterSpacing: 1.2 },
  performanceValue: { color: "#FFFFFF", fontSize: 26, fontWeight: "900", marginTop: 3 },
  performanceTrend: { fontSize: 9, fontWeight: "800", marginTop: 2 },
  performanceBadge: { width: 54, height: 54, borderRadius: 15, backgroundColor: "#1B1230", alignItems: "center", justifyContent: "center" },
  performanceBadgeValue: { color: Colors.primaryLight, fontSize: 18, fontWeight: "900" },
  performanceBadgeLabel: { color: "#777782", fontSize: 7, marginTop: 1 },
  miniChart: { height: 72, marginTop: 14, flexDirection: "row", alignItems: "flex-end", justifyContent: "space-between", paddingHorizontal: 4, borderBottomWidth: 1, borderBottomColor: "#292832" },
  bar: { width: 20, borderTopLeftRadius: 5, borderTopRightRadius: 5, backgroundColor: Colors.primary },
  chartLabels: { flexDirection: "row", justifyContent: "space-between", marginTop: 5 },
  chartLabelsText: { color: "#656570", fontSize: 7 },
  recoveryHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 2 },
  recoveryScore: { color: Colors.success, fontSize: 11, fontWeight: "900" },
  recoveryCard: { backgroundColor: "#111116", borderRadius: 18, borderWidth: 1, borderColor: "#24232D", flexDirection: "row", alignItems: "stretch", paddingVertical: 14 },
  metric: { flex: 1, alignItems: "center", gap: 4 },
  metricValue: { color: "#FFFFFF", fontSize: 17, fontWeight: "900" },
  metricLabel: { color: "#777782", fontSize: 9 },
  divider: { width: 1, backgroundColor: "#282731" },
  recordCard: { backgroundColor: "#111116", borderRadius: 18, padding: 14, borderWidth: 1, borderColor: "#24232D", flexDirection: "row", alignItems: "center" },
  recordIcon: { width: 40, height: 40, borderRadius: 13, backgroundColor: "#21143C", alignItems: "center", justifyContent: "center", marginRight: 10 },
  recordExercise: { color: "#FFFFFF", fontSize: 14, fontWeight: "800" },
  recordDate: { color: "#777782", fontSize: 9, marginTop: 3 },
  recordValue: { color: Colors.success, fontSize: 17, fontWeight: "900", marginLeft: 8 },
  emptyRecord: { backgroundColor: "#111116", borderRadius: 18, padding: 15, borderWidth: 1, borderColor: "#24232D", flexDirection: "row", alignItems: "center", gap: 10 },
  emptyRecordText: { color: "#858591", flex: 1, fontSize: 11, lineHeight: 16 },
