...
function Metric({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <View style={styles.metric}>
      <Icon color={Colors.primaryLight} size={18} />
...
function getCoachDecision(recoveryScore: number | null | undefined, hasProgram: boolean) {
  if (!hasProgram) return { eyebrow: "PREMIÈRE ÉTAPE", title: "Construis ton programme", message: "Ajoute tes exercices et Progress+ pourra commencer à piloter ta progression série par série.", action: "Voir mes programmes", tone: "neutral" as const };
  if (recoveryScore == null) return { eyebrow: "COACH PROGRESS+", title: "Prêt à décider avec tes données", message: "Fais ton check-in au début de ta prochaine séance : Progress+ adaptera ensuite l'intensité à ton état du jour.", action: "Commencer la séance", tone: "neutral" as const };
  if (recoveryScore <= 2) return { eyebrow: "COACH PROGRESS+", title: "Aujourd'hui, on consolide", message: "Ta récupération est basse. Pas de surcharge : priorité à une exécution propre et à la maîtrise de tes séries.", action: "Ouvrir ma séance", tone: "caution" as const };
  if (recoveryScore >= 4.2) return { eyebrow: "COACH PROGRESS+", title: "Feu vert pour progresser", message: "Ta récupération est très bonne. Si ta technique reste solide, Progress+ cherchera une progression contrôlée.", action: "Lancer ma séance", tone: "ready" as const };
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={Colors.primary} />} contentContainerStyle={styles.container}>
        <View style={styles.header}>
          <View style={{ flex: 1 }}><Text style={styles.eyebrow}>PROGRESS+</Text><Text style={styles.greeting}>Bonjour {firstName} 👋</Text><Text style={styles.subtitle}>On continue à construire ta progression.</Text></View>
          <TouchableOpacity style={styles.avatar} onPress={() => router.push("/(app)/profile")}><Text style={styles.avatarText}>{firstName.charAt(0).toUpperCase()}</Text></TouchableOpacity>
        </View>

        <TouchableOpacity style={styles.levelCard} activeOpacity={0.9} onPress={() => router.push("/(app)/ranking")}>
          <View style={styles.levelTop}><View><Text style={styles.levelLabel}>{ranking?.rank?.toUpperCase() ?? "FER"}</Text><Text style={styles.levelTitle}>{ranking?.score ?? 0} points</Text></View><View style={styles.xpPill}><Sparkles color={Colors.primaryLight} size={15} /><Text style={styles.xpText}>{ranking?.season_points ?? 0} saison</Text></View></View>
          <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${Math.round((rankProgress?.percent ?? 0) * 100)}%` }]} /></View>
        ...
        <TouchableOpacity style={styles.progressCard} onPress={() => router.push("/(app)/progress")}>
          <View style={styles.progressTitleRow}><View><Text style={[styles.progressBig, trendPositive && styles.successText, trendNegative && styles.dangerText]}>{hasComparison ? `${volumeChange >= 0 ? "+" : ""}${volumeChange.toFixed(1)}%` : formatVolume(data?.monthVolume ?? 0)}</Text><Text style={styles.progressLabel}>{hasComparison ? "volume vs mois précédent" : "volume ce mois-ci"}</Text></View><View style={[styles.trendBadge, trendPositive && styles.trendPositive, trendNegative && styles.trendNegative]}><Text style={[styles.trendText, trendPositive && styles.successText, trendNegative && styles.dangerText]}>{hasComparison ? (volumeChange >= 0 ? "↗ EN HAUSSE" : "↘ EN BAISSE") : "EN CONSTRUCTION"}</Text></View></View>
          <View style={styles.progressSummary}><Text style={styles.progressSummaryValue}>{data?.history?.length ?? 0}</Text><Text style={styles.progressSummaryLabel}>séances enregistrées</Text></View>
        </TouchableOpacity>

        <SectionHeader title="Dernier record" />
        {record ? <View style={styles.recordCard}><View style={{ flex: 1 }}><Text style={styles.recordExercise}>{record.exercises?.name ?? "Exercice"}</Text><Text style={styles.recordDate}>Meilleur 1RM estimé : {Number(record.estimated_1rm ?? 0).toLocaleString("fr-FR")} kg</Text></View><Text style={styles.recordValue}>{Number(record.weight).toLocaleString("fr-FR")} × {record.reps}</Text></View> : <TouchableOpacity style={styles.emptyRecord} onPress={() => program && router.push({ pathname: "/(app)/program/[id]", params: { id: program.id } })}><Trophy color={Colors.primary} size={21} /><Text style={styles.emptyRecordText}>Ton premier record apparaîtra après une série enregistrée.</Text></TouchableOpacity>}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.primary, alignItems: "center", justifyContent: "center", marginLeft: 12 },
  avatarText: { color: Colors.primaryLight, fontSize: 20, fontWeight: "900" },
  ...
  levelLabel: { color: Colors.primaryLight, fontSize: 11, fontWeight: "900", letterSpacing: 1.5 },
  levelTitle: { color: Colors.text, fontSize: 22, fontWeight: "900", marginTop: 2 },
  xpPill: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: Colors.background, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 12, borderWidth: 1, borderColor: Colors.border },
  ...
  sectionAction: { color: Colors.primaryLight, fontSize: 12, fontWeight: "800" },
  coachCard: { backgroundColor: Colors.surface, borderRadius: 20, padding: 16, borderWidth: 1, flexDirection: "row", alignItems: "flex-start" },
  coachIcon: { width: 44, height: 44, borderRadius: 14, backgroundColor: Colors.background, alignItems: "center", justifyContent: "center", borderWidth: 1 },
  coachContent: { flex: 1, marginLeft: 12 },
  coachEyebrowRow: { flexDirection: "row", alignItems: "center", gap: 7 },
  coachEyebrow: { fontSize: 10, fontWeight: "900", letterSpacing: 1.2 },
  ...
  coachActionText: { fontSize: 12, fontWeight: "900" },
  ...
  workoutType: { color: Colors.primaryLight, fontSize: 20, fontWeight: "900" },
  ...
  trendBadge: { backgroundColor: Colors.background, paddingHorizontal: 9, paddingVertical: 6, borderRadius: 9, borderWidth: 1, borderColor: Colors.border },
  trendPositive: { backgroundColor: "#102A1B", borderColor: Colors.success },
  trendNegative: { backgroundColor: "#32151D", borderColor: Colors.danger },
  trendText: { color: Colors.primaryLight, fontSize: 10, fontWeight: "900" },
  ...
  recordValue: { color: Colors.success, fontSize: 20, fontWeight: "900", marginLeft: 10 },
