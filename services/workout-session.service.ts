...
export async function saveWorkoutSet(
  ...
  if (weight <= 0 || reps <= 0 || setNumber <= 0) {
    throw new Error("Les valeurs de la série sont invalides.");
  }

  // Idempotent : si l'utilisateur valide deux fois après une erreur réseau,
  // on met à jour la série existante au lieu de créer un doublon.
...
export async function getProgressionRecommendation(
  ...
  if (!latestSet) {
    return { action: "start", currentWeight: 0, recommendedWeight: 0, currentReps: minReps, recommendedReps: minReps, minReps, maxReps, completedSets: 0, message: `Commence à ${minReps}–${maxReps} répétitions et trouve ton poids de travail.` };
  }
...
export async function getPersonalRecords(userId: string, limit = 10): Promise<PersonalRecordItem[]> {
  const { data, error } = await supabase
    .from("personal_records")
    .select(`id, exercise_id, weight, reps, estimated_1rm, created_at, exercises (name)`)
    .eq("user_id", userId)
...
export async function getWorkoutHistory(userId: string, limit = 20): Promise<WorkoutHistoryItem[]> {
  const { data, error } = await supabase
    .from("workout_sessions")
    .select(`id, program_id, started_at, finished_at, duration_seconds, total_volume, total_sets, workout_programs (name)`)
    .eq("user_id", userId)
