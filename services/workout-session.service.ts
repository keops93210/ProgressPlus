...
export async function getProgressionRecommendation(userId: string | null, exerciseId: string, minReps: number, maxReps: number, plannedSets: number): Promise<ProgressionRecommendation> {
  ...
  if (!sets.length) return { action: "start", currentWeight: 0, recommendedWeight: 0, currentReps: minReps, recommendedReps: minReps, minReps, maxReps, completedSets: 0, message: `Commence à ${minReps}–${maxReps} répétitions et trouve ton poids de travail.` };
