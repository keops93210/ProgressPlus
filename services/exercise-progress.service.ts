...
export type ExerciseProgressSummary = {
  ...
  personalRecord: { weight: number; reps: number; estimated1rm: number } | null;
};
...
export async function getExerciseProgress(
  ...
  for (const row of data ?? []) {
    ...
    if (!existing) {
      grouped.set(sessionId, { sessionId, date: String(row.created_at), weight, reps, volume: weight * reps, estimated1rm });
      continue;
    }
  ...
  const trendPercent = oldest && latest && oldest.estimated1rm > 0
    ? Math.round(((latest.estimated1rm - oldest.estimated1rm) / oldest.estimated1rm) * 100)
    : 0;
  ...
  return {
    points,
    summary: { bestWeight, bestReps, bestEstimated1rm, bestVolume, sessionsCount: points.length, trendPercent, trend, personalRecord: personalRecord ? { weight: Number(personalRecord.weight) || 0, reps: Number(personalRecord.reps) || 0, estimated1rm: Number(personalRecord.estimated_1rm) || 0 } : null },
  };
}
