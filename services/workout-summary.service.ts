...
export async function getWorkoutSummary(sessionId: string): Promise<WorkoutSummary> {
  const { data: session, error: sessionError } = await supabase
    ...
    .select("id, user_id, program_id, recovery_score, duration_seconds, total_volume, total_sets, finished_at")
    .eq("id", sessionId)
    .single();
  ...
  if (session.program_id) {
    const { data: plannedExercises, error: plannedError } = await supabase
  ...
  else if (completionPercent < 100) nextSessionAdvice = "Certaines séries n'ont pas été terminées : consolide la charge et reconstruis progressivement le volume.";
  ...
  return {
    ...
    qualityScore,
    completionPercent,
    recoveryScore: session.recovery_score == null ? null : Number(session.recovery_score),
    message,
    nextSessionAdvice,
  };
}
