...
export function estimateOneRepMax(weight: number, reps: number) { return weight <= 0 || reps <= 0 ? 0 : weight * (1 + reps / 30); }

export function normalizeRir(rir?: number | null, rpe?: number | null) {
  if (typeof rir === "number" && Number.isFinite(rir)) return clamp(rir, 0, 5);
  if (typeof rpe === "number" && Number.isFinite(rpe)) return clamp(10 - rpe, 0, 5);
  return null;
}
...
export function getSuggestedRestSeconds(rir: number, baseRestSeconds: number, readiness?: number | null) {
  ...
  const readinessMultiplier = readiness != null && readiness <= 2 ? 1.15 : 1;
  return Math.round((base * effortMultiplier * readinessMultiplier) / 15) * 15;
}

export function getTrainingQualityScore(input: { completedSets: number; plannedSets: number; averageRir?: number | null; readiness?: number | null; trendPercent?: number | null; personalRecords?: number }) {
